...
class TGNNRegressor(nn.Module):
    """Paper-aligned TGNN backbone: 2xGCN + 2xLSTM + FC"""
    ...
    def encode_graph_step(self, x_step, edge_index, edge_weight):
        ...
        h = self.gcn2(h, edge_index, edge_weight)  # no ReLU after last GCN per paper style
        return h.reshape(-1)  # [6*gcn_hidden]

    def forward(self, x_seq, m_seq, edge_index_np, device):
        # x_seq: [B, W, 6], m_seq: [B, W]
        ...
        for b in range(bsz):
            ...
            for t in range(win):
                ...
                step_embs.append(z)
            step_embs = torch.stack(step_embs, dim=0)  # [W, 6*gcn_hidden]
            seq_embeddings.append(step_embs)

        seq_embeddings = torch.stack(seq_embeddings, dim=0)  # [B, W, 6*gcn_hidden]
        out, _ = self.lstm(seq_embeddings)
        h_last = out[:, -1, :]
...
def main():
    parser = argparse.ArgumentParser(description="Train TGNN (2xGCN + 2xLSTM) on seq6")
    ...
    parser.add_argument("--train_limit", type=int, default=0, help="0 means use all train windows")
    parser.add_argument("--test_limit", type=int, default=0, help="0 means use all test windows")
    parser.add_argument("--log_every", type=int, default=10)
    parser.add_argument("--model_out", default="/home/evawang/T-GCN/data/auv_tgnn_seq6_model.pt")
    parser.add_argument("--history_out", default="/home/evawang/T-GCN/data/auv_tgnn_seq6_history.csv")
    parser.add_argument("--curve_out", default="/home/evawang/T-GCN/data/auv_tgnn_seq6_loss_curve.png")
    ...
    if args.train_limit > 0:
        Xw_train = Xw_train[: args.train_limit]
        Mw_train = Mw_train[: args.train_limit]
        Yw_train = Yw_train[: args.train_limit]
    if args.test_limit > 0:
        Xw_test = Xw_test[: args.test_limit]
        Mw_test = Mw_test[: args.test_limit]
        Yw_test = Yw_test[: args.test_limit]
    ...
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
    plt.title("TGNN Training Curve (RMSE vs Epoch)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
