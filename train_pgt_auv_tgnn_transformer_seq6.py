...
class TGNNTransformerRegressor(nn.Module):
    """Paper-style spatial+temporal: 2xGCN + TransformerEncoder + FC"""
    ...
    def forward(self, x_seq, m_seq, edge_index_np, device):
        ...
        seq_embeddings = torch.stack(seq_embeddings, dim=0)  # [B,W,F]
        seq_embeddings = self.proj(seq_embeddings)
        seq_embeddings = self.pos(seq_embeddings)

        h = self.encoder(seq_embeddings)  # [B,W,D]
        h_last = h[:, -1, :]
...
def main():
    parser = argparse.ArgumentParser(description="Train TGNN-Transformer on seq6")
    ...
    parser.add_argument("--tf_ff", type=int, default=256)
    parser.add_argument("--dropout", type=float, default=0.2)
    parser.add_argument("--train_limit", type=int, default=0)
    parser.add_argument("--test_limit", type=int, default=0)
    parser.add_argument("--log_every", type=int, default=10)
    parser.add_argument("--model_out", default="/home/evawang/T-GCN/data/auv_tgnn_transformer_seq6_model.pt")
    parser.add_argument("--history_out", default="/home/evawang/T-GCN/data/auv_tgnn_transformer_seq6_history.csv")
    parser.add_argument("--curve_out", default="/home/evawang/T-GCN/data/auv_tgnn_transformer_seq6_loss_curve.png")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("RMSE")
    plt.title("TGNN-Transformer Training Curve (RMSE vs Epoch)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
