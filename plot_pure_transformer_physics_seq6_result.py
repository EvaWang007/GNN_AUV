...
def main():
    ...
    parser.add_argument("--model_path", default="/home/evawang/T-GCN/data/auv_pure_tf_phy_seq6_model.pt")
    ...
    plt.plot(preds[:, 0], preds[:, 1], label="Pure Transformer+Physics")
    plt.title("Pure Transformer+Physics Trajectory")
    plt.xlabel("x (m)")
    plt.ylabel("y (m)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    p1 = f"{args.out_dir}/plot_traj_pure_tf_phy_seq6{args.suffix}.png"
    plt.savefig(p1, dpi=170)
    plt.close()

    plt.figure(figsize=(6, 6))
    plt.plot(Yw[:, 0], Yw[:, 1], label="COLMAP Ground Truth")
    plt.plot(preds_aligned[:, 0], preds_aligned[:, 1], label="Pure TF+Physics (Aligned)")
    plt.title("Pure Transformer+Physics Trajectory (Aligned)")
    plt.xlabel("x (m)")
    plt.ylabel("y (m)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    p2 = f"{args.out_dir}/plot_traj_pure_tf_phy_seq6_aligned{args.suffix}.png"
    plt.savefig(p2, dpi=170)
    plt.close()

    plt.figure(figsize=(8, 3.5))
    plt.plot(err)
    plt.title("Pure Transformer+Physics Per-step Error")
    plt.xlabel("test step")
    plt.ylabel("error (m)")
    plt.grid(True)
    plt.tight_layout()
    p3 = f"{args.out_dir}/plot_error_pure_tf_phy_seq6{args.suffix}.png"
