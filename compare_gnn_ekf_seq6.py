...
def main():
    ...
    gt_xy = gt
    n = min(len(gt_xy), len(pred_e1), len(pred_e2))
    gt_xy = gt_xy[:n]
    pred_gnn = pred_gnn[:n]
    pred_e1 = pred_e1[:n]
    pred_e2 = pred_e2[:n]
    ...
    plt.plot(pred_gnn[:, 0], pred_gnn[:, 1], label='GCN-6D')
    plt.plot(pred_e1[:, 0], pred_e1[:, 1], label='EKF-Single')
    plt.plot(pred_e2[:, 0], pred_e2[:, 1], label='EKF-Joint')
    plt.title('Trajectory Comparison (Seq6)')
    plt.xlabel('x (m)')
    plt.ylabel('y (m)')
    plt.axis('equal')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    p1 = out_dir / 'compare_traj_seq6.png'
    ...
    for err, name in [(err_g, 'GCN-6D'), (err_e1, 'EKF-Single'), (err_e2, 'EKF-Joint')]:
        sx, sy = cdf_xy(err)
    ...
    # Bar chart RMSE/ADE
    names = ['GCN-6D', 'EKF-Single', 'EKF-Joint']
    ade = [m_gnn['mean_error'], m_e1['mean_error'], m_e2['mean_error']]
    rmse = [m_gnn['rmse'], m_e1['rmse'], m_e2['rmse']]
    ...
    for name, m in [('GCN-6D', m_gnn), ('EKF-Single', m_e1), ('EKF-Joint', m_e2)]:
